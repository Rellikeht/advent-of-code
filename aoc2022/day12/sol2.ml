...
let rec mark map marks w h endp pos v moves inf =
    ...
    for i = 0 to endval do
        ...
        && get_arr marks npos w > v+1 then begin
            let nv = if get_map map npos w == 'a' then 0 else v+1 in
            set_arr marks npos w nv;
            Queue.add (npos, nv) moves
        end else ()
    done;
...
let rec print_marks marks w h x y =
    if y == h then ()
    else if x == w then begin
        print_char '\n';
        print_marks marks w h 0 (y+1)
    end else begin
        print_int (get_arr marks (x, y) w);
        print_char ' ';
        print_marks marks w h (x+1) y
    end
;;
print_marks marks width height 0 0;

let print_pos (x, y) =
    print_int x; print_char ' '; print_int y; print_char '\n'
;;

let eq (a, b) (c, d) = a == c && b == d;;
