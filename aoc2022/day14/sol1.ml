...
let rec drop map w bot (x, y) =
    if y >= bot then false
        ...
        | None -> begin
            set_at map x y w Sand; true
        end
        | Some nx -> drop map w bot (nx, ny)
    end
;;

let rec drops map w bot stp count =
    if drop map w bot stp then
        drops map w bot stp (count+1)
    else count
;;
